...
from recklock.policy import evaluate_action, load_action_request_yaml, load_policies_yaml
...
from recklock.gateway import (
    execute_request,
    load_execution_request_yaml,
    load_registry_index,
)
from recklock.discovery.evidence import build_evidence_report, write_evidence_reports
from recklock.discovery.manifest_generator import compute_agent_id, write_manifest_draft
...
def create_api_key_cmd(
    ...
    database_url: str | None = typer.Option(
        None,
        "--database-url",
        help="SQLAlchemy URL (defaults to DATABASE_URL / RECKLOCK_DATABASE_URL / configured SQLite)",
    ),
...
def request_credential_cmd(
    ...
    from recklock.api.settings import resolve_paths
    from recklock.credentials.broker import issue_credential
    from recklock.credentials.models import load_credential_request_yaml
    from recklock.gateway import load_registry_index
    from recklock.policy import load_policies_yaml
...
def revoke_credential_cmd(
    ...
    from recklock.credentials.broker import revoke_credential
...
def verify_credential_cmd(
    ...
    from recklock.credentials.broker import verify_credential
...
def serve_cmd(
    host: str | None = typer.Option(
        None,
        "--host",
        help="Bind address (default: RECKLOCK_API_HOST or 127.0.0.1)",
    ),
    port: int | None = typer.Option(
        None,
        "--port",
        help="TCP port (default: RECKLOCK_API_PORT or 8080)",
    ),
    ...
    from recklock.api.app import create_app
    from recklock.api.settings import ApiSettings
