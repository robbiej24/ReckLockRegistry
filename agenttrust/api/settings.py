...
from agenttrust.audit import DEFAULT_AUDIT_LOG_PATH
...
class ApiSettings(BaseSettings):
    """Resolved from environment with prefix ``AGENTTRUST_``."""

    model_config = SettingsConfigDict(
        env_prefix="AGENTTRUST_",
        env_file_encoding="utf-8",
        extra="ignore",
    )

    database_url: str = Field(
        default="sqlite:///./agenttrust.local.db",
        description="SQLAlchemy URL (override with DATABASE_URL or AGENTTRUST_DATABASE_URL).",
    )
    env: str = Field(
        default="development",
        description="Deployment environment label (AGENTTRUST_ENV).",
    )
    api_host: str = Field(
        default="127.0.0.1",
        description="Default bind address for ``recklock-registry serve`` (AGENTTRUST_API_HOST).",
    )
    api_port: int = Field(
        default=8080,
        description="Default TCP port for ``recklock-registry serve`` (AGENTTRUST_API_PORT).",
    )
    secret_key: str | None = Field(
        default=None,
        description="Optional signing secret reserved for future use (AGENTTRUST_SECRET_KEY).",
    )
    ...
    observation_mode: bool = Field(
        default=False,
        description="When True, API telemetry endpoints persist events (AGENTTRUST_OBSERVATION_MODE).",
    )
