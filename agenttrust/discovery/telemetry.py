...
from agenttrust.constants import DEFAULT_EVIDENCE_DIR
...
def observation_mode_enabled(*, force: bool = False) -> bool:
    if force:
        return True
    raw = os.environ.get("AGENTTRUST_OBSERVATION_MODE", "")
...
def resolve_observation_log_path(events_path: Path | None = None) -> Path:
    """Default log path under cwd or ``AGENTTRUST_EVIDENCE_DIR``."""
    if events_path is not None:
        p = events_path
        return p.resolve() if p.is_absolute() else (Path.cwd() / p).resolve()
    env_dir = os.environ.get("AGENTTRUST_EVIDENCE_DIR")
...
def _append_event(
    ...
    *,
    ...
    try:
        ...
        _LOG.debug("agenttrust observation append failed: %s", exc)
