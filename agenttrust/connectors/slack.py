...
from agenttrust.connectors.base import BaseConnector, ConnectorRequest, ConnectorResponse, real_connectors_enabled
    ...
    def execute(self, request: ConnectorRequest) -> ConnectorResponse:
        if not real_connectors_enabled():
            return ConnectorResponse(
                ...
                dry_run=False,
                success=False,
                message="Real Slack execution disabled (set AGENTTRUST_ENABLE_REAL_CONNECTORS=true to enable).",
            )
        self.validate_config(request.config)
        return ConnectorResponse(
            connector_id=self.connector_id,
